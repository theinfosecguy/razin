...
class TestStdoutReporterHeader:
    ...
    def test_header_shows_file_count(self) -> None:
    ...
    def test_header_shows_duration(self) -> None:
        result = _make_result(duration=2.567)
        output = StdoutReporter(result, color=False).render()
        assert "2.567s" in output

    def test_header_shows_inline_severity_breakdown(self) -> None:
    ...
    def test_cache_hidden_by_default(self) -> None:
        result = _make_result(cache_hits=10, cache_misses=5)
        output = StdoutReporter(result, color=False).render()
        assert "hits" not in output

    def test_cache_shown_in_verbose(self) -> None:
        result = _make_result(cache_hits=10, cache_misses=5)
    ...
    def test_header_has_separator_line(self) -> None:
        result = _make_result()
        output = StdoutReporter(result, color=False).render()
        assert "\u2500" in output


class TestStdoutReporterFindingsTable:
    def test_findings_table_has_borders(self) -> None:
        findings = [
    ...
    def test_findings_table_shows_all_findings(self) -> None:
        all_findings = [
            _make_finding(skill="evil-skill", rule_id="SECRET_REF", score=90),
    ...
    def test_findings_table_hidden_when_empty(self) -> None:
...
class TestStdoutReporterColors:
    def test_high_severity_colored_red(self) -> None:
        f = [_make_finding(severity="high", score=80)]
        result = _make_result(findings=f)
        output = StdoutReporter(result, color=True).render()
        assert _RED in output
        assert _RESET in output

    def test_medium_severity_colored_yellow(self) -> None:
        f = [_make_finding(severity="medium", score=55, finding_id="med1")]
        result = _make_result(findings=f)
        output = StdoutReporter(result, color=True).render()
        assert _YELLOW in output

    def test_low_severity_colored_green(self) -> None:
        f = [_make_finding(severity="low", score=20, finding_id="low1")]
        result = _make_result(findings=f)
        output = StdoutReporter(result, color=True).render()
        assert _GREEN in output

    def test_high_score_colored_red(self) -> None:
        f = [_make_finding(score=85)]
        result = _make_result(findings=f)
        output = StdoutReporter(result, color=True).render()
        assert f"{_RED}85{_RESET}" in output

    def test_medium_score_colored_yellow(self) -> None:
        f = [_make_finding(score=50, severity="medium", finding_id="m1")]
        result = _make_result(findings=f)
        output = StdoutReporter(result, color=True).render()
        assert f"{_YELLOW}50{_RESET}" in output

    def test_low_score_colored_green(self) -> None:
        f = [_make_finding(score=20, severity="low", finding_id="l1")]
        result = _make_result(findings=f)
        output = StdoutReporter(result, color=True).render()
        assert f"{_GREEN}20{_RESET}" in output

    def test_color_disabled_produces_no_ansi(self) -> None:
        f = [_make_finding(score=85)]
        result = _make_result(findings=f, counts={"high": 1, "medium": 0, "low": 0})
        output = StdoutReporter(result, color=False).render()
        assert "\033[" not in output

    def test_risk_score_colored_in_header(self) -> None:
        result = _make_result(aggregate_score=94, aggregate_severity="high")
        output = StdoutReporter(result, color=True).render()
        assert _RED in output


class TestStdoutReporterDeterminism:
...
class TestStdoutReporterGolden:
    def test_golden_snapshot_no_color(self) -> None:
